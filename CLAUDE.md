...
| Тесты | `cd backend && uv run pytest -q` — **62 passed** |
...
- **Регион в WB/Ozon/Runet** — параметр принимается, но игнорируется. Для WB нужна
  таблица `dest`-кодов по городам.
...
- **nodriver — AGPL-3.0**, конфликтует с MIT при импорте в код. Изолирован в
  опциональной extra `stealth` + ленивый импорт.
