...
## Статус по ТЗ (аудит 22.05.2026)

**Закрыто**: 4 источника, группировка + счётчики + min-цена, fan-out с изоляцией,
SSE на бэке, anti-bot слой, free-mode флаги, sentiment-анализ, Prometheus-метрики.

**Открытые дыры (P0 — прямые несоответствия ТЗ)**:
- **Синонимы** — `enrichment/synonyms.py` делает только транслитерацию брендов,
  настоящих синонимов (телефон≈смартфон) нет. Нужен тезаурус + морфология.
- **Опечатки** — `enrichment/typos.py` правит только бренды (словарь ~60 слов),
  общие названия товаров не покрыты.
- **avg/median по группе** — `SourceGroup.median_price` объявлено, не вычисляется.
