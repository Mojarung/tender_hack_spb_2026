"""Unit tests for the SpellCheck client + its integration into normalize_query."""
...
_TEST_URL = "http://spellcheck-test:8080"


def test_client_disabled_when_url_is_empty() -> None:
    assert SpellCheckClient(url="").enabled is False


def test_client_enabled_when_url_present() -> None:
    assert SpellCheckClient(url=_TEST_URL).enabled is True


async def test_fix_returns_none_when_disabled() -> None:
    assert await SpellCheckClient(url="").fix("anything") is None


async def test_fix_returns_none_for_empty_text() -> None:
    client = SpellCheckClient(url=_TEST_URL)
...
async def test_fix_returns_none_on_http_5xx() -> None:
    ...
    assert await SpellCheckClient(url=_TEST_URL).fix("anything") is None
...
@respx.mock
async def test_normalize_query_applies_spellcheck_correction() -> None:
    respx.post(f"{_TEST_URL}/fix").mock(
        return_value=httpx.Response(
            200,
            json={
    ...
    n = await normalize_query(
        ...
        spellcheck=SpellCheckClient(url=_TEST_URL),
    )
...
@respx.mock
async def test_normalize_query_unchanged_when_spellcheck_unreachable() -> None:
    respx.post(f"{_TEST_URL}/fix").mock(
